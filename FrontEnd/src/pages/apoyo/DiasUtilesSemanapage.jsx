...
export default function DiasUtilesSemanaPage() {
  ...
  const loadRows = async () => {
    ...
    try {
      setLoading(true);
      const res = await auth.get(`/dias-utiles-semana/?empresaId=${empresaId}`);
      setRows(res.data || []);
    } catch (err) {
  ...
  const handleCreate = async (data) => {
    try {
      await auth.post("/dias-utiles-semana/", {
        ...data,
  ...
  const handleUpdate = async (data) => {
    try {
      await auth.put(`/dias-utiles-semana/${data.PKID}`, {
        ...data,
        Ano: Number(data.Ano),
        Semana: Number(data.Semana),
        NumeroDiasUtiles:
  ...
  const handleDelete = async (row) => {
    ...
    try {
      await auth.delete(`/dias-utiles-semana/${row.PKID}`);
      await loadRows();
    } catch (err) {
      alert(prettyError(err));
    }

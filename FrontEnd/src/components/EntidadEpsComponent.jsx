// archivo: src/components/EntidadEpsComponent.jsx
...
export default function EntidadEpsComponent() {
  ...
  const loadRows = async () => {
    try {
      ...
      const url = qs.toString() ? `/entidad-eps/?${qs.toString()}` : `/entidad-eps/`;
  ...
  const onDelete = async (id) => {
    ...
    try {
      await auth.delete(`/entidad-eps/${id}`);
      await loadRows();
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const payload = {
      IDEntidadEps: Number(form.IDEntidadEps),
      EntidadEps: String(form.EntidadEps || "").trim(),
      PKIDSituacionRegistro: Number(form.PKIDSituacionRegistro),
    };
    try {
      if (isEditing) {
        await auth.put(`/entidad-eps/${form.PKID}`, payload);
        alert("Actualizado.");
      } else {
        await auth.post(`/entidad-eps/`, payload);
        alert("Creado.");
      }
      await loadRows();
      onNew();
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ marginBottom: 12 }}>Entidad EPS</h2>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <form onSubmit={onSubmit}>
          <div style={row4}>
            <div>
              <label style={label}>ID Entidad EPS *</label>
              <input name="IDEntidadEps" value={form.IDEntidadEps} onChange={onChange} style={input} />
            </div>
            <div>
              <label style={label}>Entidad EPS *</label>
              <input name="EntidadEps" value={form.EntidadEps} onChange={onChange} style={input} />
            </div>
            <div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f9fafb" }}>
                ...
                <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #e5e7eb" }}>Entidad EPS</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.PKID} style={selected?.PKID === r.PKID ? { background: "#fef3c7" } : undefined}>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.IDEntidadEps}</td>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.EntidadEps}</td>

// archivo: src/components/GradoAcademicoComponent.jsx
...
export default function GradoAcademicoComponent() {
  ...
  const loadSituaciones = async () => {
    try {
      const r = await auth.get("/grado-academico-combos/situacion");
  ...
  const loadRows = async () => {
    try {
      const res = await auth.get(`/grado-academico/?${buildQuery(true)}`);
      setRows(res.data?.items || []);
      setTotal(res.data?.total ?? 0);
    } catch (err) {
      console.error(err);
      alert("No se pudo listar Grado Académico.");
    }
  };
  ...
  const onDelete = async (id) => {
    ...
    try {
      await auth.delete(`/grado-academico/${id}`);
      await loadRows();
      onNew();
    } catch (err) {
  ...
  const validateClient = () => {
    if (!form.IDGradoAcademico) return "IDGradoAcademico es obligatorio.";
    if (!/^-?\d+$/.test(String(form.IDGradoAcademico))) return "IDGradoAcademico debe ser entero.";
    if (!String(form.GradoAcademico || "").trim()) return "GradoAcademico es obligatorio.";
    if (String(form.GradoAcademico).length > 50) return "GradoAcademico máximo 50 caracteres.";
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      IDGradoAcademico: Number(form.IDGradoAcademico),
      GradoAcademico: String(form.GradoAcademico || "").trim(),
      PKIDSituacionRegistro: Number(form.PKIDSituacionRegistro),
    };
    try {
      if (isEditing) {
        await auth.put(`/grado-academico/${form.PKID}`, payload);
        alert("Actualizado.");
      } else {
        await auth.post(`/grado-academico/`, payload);
        alert("Creado.");
      }
      await loadRows();
      onNew();
  ...
  const exportCSV = async () => {
    try {
      ...
      const res = await auth.get(`/grado-academico/export?${qs}`, { responseType: "blob" });
      const blob = new Blob([res.data], { type: "text/csv;charset=utf-8" });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "grado_academico.csv";
      a.click();
  ...
  const exportXLSX = async () => {
    try {
      const qs = buildQuery(false);
      const res = await auth.get(`/grado-academico/export-xlsx?${qs}`, { responseType: "blob" });
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = "grado_academico.xlsx";
      a.click();
      window.URL.revokeObjectURL(url);
    } catch {
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ marginBottom: 12 }}>Grado Académico</h2>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <form onSubmit={onSubmit}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            <div>
              <label style={label}>ID Grado Académico *</label>
              <input
                name="IDGradoAcademico"
                value={form.IDGradoAcademico}
                onChange={onChange}
                style={input}
              />
            </div>
            <div>
              <label style={label}>Grado Académico *</label>
              <input
                name="GradoAcademico"
      ...
      <div style={card}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            />
            <span style={{ fontSize: 12 }}>/ {totalPages}</span>
            <select
              value={pageSize}
              onChange={(e) => {
            ...
            <button
              ...
              style={{ ...btn.base, ...btn.neutral, opacity: page >= totalPages ? 0.5 : 1 }}
            >
              ▶
            </button>
          </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f9fafb" }}>
                ...
                <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #e5e7eb" }}>Grado Académico</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.PKID}>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.IDGradoAcademico}</td>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.GradoAcademico}</td>

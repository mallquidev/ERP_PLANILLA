// archivo: src/components/GrupoGastoComponent.jsx
...
export default function GrupoGastoComponent() {
  ...
  const loadSituaciones = async () => {
    try {
      const r = await auth.get("/grupo-gasto-combos/situacion");
  ...
  const loadRows = async () => {
    try {
      const res = await auth.get(`/grupo-gasto/?${buildQuery(true)}`);
      setRows(res.data?.items || []);
      setTotal(res.data?.total ?? 0);
    } catch (err) {
      console.error(err);
      alert("No se pudo listar Grupo Gasto.");
    }
  };
  ...
  const onDelete = async (id) => {
    ...
    try {
      await auth.delete(`/grupo-gasto/${id}`);
      await loadRows();
      onNew();
    } catch (err) {
  ...
  const validateClient = () => {
    if (!form.PKIDGrupoGasto) return "PKIDGrupoGasto es obligatorio.";
    if (!/^-?\d+$/.test(String(form.PKIDGrupoGasto))) return "PKIDGrupoGasto debe ser entero.";
    if (!String(form.GrupoGasto || "").trim()) return "GrupoGasto es obligatorio.";
    if (String(form.GrupoGasto).length > 50) return "GrupoGasto máximo 50 caracteres.";
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      PKIDGrupoGasto: Number(form.PKIDGrupoGasto),
      GrupoGasto: String(form.GrupoGasto || "").trim(),
      PKIDSituacionRegistro: Number(form.PKIDSituacionRegistro),
    };
    try {
      if (isEditing) {
        await auth.put(`/grupo-gasto/${form.PKID}`, payload);
        alert("Actualizado.");
      } else {
        await auth.post(`/grupo-gasto/`, payload);
        alert("Creado.");
      }
      await loadRows();
      onNew();
  ...
  const exportCSV = async () => {
    try {
      ...
      const res = await auth.get(`/grupo-gasto/export?${qs}`, { responseType: "blob" });
      const blob = new Blob([res.data], { type: "text/csv;charset=utf-8" });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "grupo_gasto.csv";
      a.click();
  ...
  const exportXLSX = async () => {
    try {
      const qs = buildQuery(false);
      const res = await auth.get(`/grupo-gasto/export-xlsx?${qs}`, { responseType: "blob" });
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ marginBottom: 12 }}>Grupo de Gasto</h2>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <form onSubmit={onSubmit}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            <div>
              <label style={label}>PKID Grupo Gasto *</label>
              <input
                name="PKIDGrupoGasto"
                value={form.PKIDGrupoGasto}
                onChange={onChange}
                style={input}
              />
            </div>
            <div>
              <label style={label}>Grupo Gasto *</label>
              <input
                name="GrupoGasto"
                value={form.GrupoGasto}
                onChange={onChange}
                style={input}
                maxLength={50}
              />
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.PKID}>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.PKIDGrupoGasto}</td>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.GrupoGasto}</td>

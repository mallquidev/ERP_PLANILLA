// archivo: src/components/FamiliaComponent.jsx
...
export default function FamiliaComponent() {
  ...
  const empty = {
    ...
    IDFamilia: "",
    Familia: "",
    AcumulativaCheck: false,
    FijaCheck: false,
    CompuestaCheck: false,
  ...
  const loadSituaciones = async () => {
    try {
      const r = await auth.get(`/familia-combos/situacion/`);
  ...
  const buildQuery = (withPaging = true) => {
    ...
    if (fId) qs.append("id_familia", fId);
  ...
  const loadRows = async () => {
    try {
      const url = `/familia/?${buildQuery(true)}`;
  ...
  const onEdit = (r) => {
    ...
    setForm({
      ...
      Familia: r.Familia ?? "",
      AcumulativaCheck: Boolean(r.AcumulativaCheck),
      FijaCheck: Boolean(r.FijaCheck),
      CompuestaCheck: Boolean(r.CompuestaCheck),
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      Familia: String(form.Familia || "").trim(),
      AcumulativaCheck: !!form.AcumulativaCheck,
      FijaCheck: !!form.FijaCheck,
      CompuestaCheck: !!form.CompuestaCheck,
      PKIDSituacionRegistro: Number(form.PKIDSituacionRegistro),
    };
    try {
      if (isEditing) {
        await auth.put(`/familia/${form.PKID}`, payload);
        alert("Actualizado.");
      } else {
        await auth.post(`/familia/`, payload);
        alert("Creado.");
      }
      await loadRows();
      onNew();
  ...
  const exportCSV = async () => {
    try {
      const qs = buildQuery(false); // sin paginación, con filtros
      const res = await auth.get(`/familia/export?${qs}`, { responseType: "blob" });
  ...
  const exportXLSX = async () => {
    try {
      const qs = buildQuery(false); // sin paginación, con filtros
      const res = await auth.get(`/familia/export-xlsx?${qs}`, { responseType: "blob" });
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ marginBottom: 12 }}>Familias</h2>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <form onSubmit={onSubmit}>
          <div style={row4}>
            <div>
              <label style={label}>ID Familia *</label>
              <input name="IDFamilia" value={form.IDFamilia} onChange={onChange} style={input} />
            </div>
            <div>
              <label style={label}>Familia *</label>
              <input name="Familia" value={form.Familia} onChange={onChange} style={input} />
            </div>
            <div>
          ...
          <div style={{ display: "flex", gap: 24, marginTop: 12 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input
                type="checkbox"
                name="AcumulativaCheck"
                checked={!!form.AcumulativaCheck}
                onChange={onChange}
              />
              Acumulativa
            </label>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f9fafb" }}>
                ...
                <th style={{ textAlign: "center", padding: 8, borderBottom: "1px solid #e5e7eb" }}>Acum.</th>
                <th style={{ textAlign: "center", padding: 8, borderBottom: "1px solid #e5e7eb" }}>Fija</th>
                <th style={{ textAlign: "center", padding: 8, borderBottom: "1px solid #e5e7eb" }}>Comp.</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.PKID} style={selected?.PKID === r.PKID ? { background: "#fef3c7" } : undefined}>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.IDFamilia}</td>
                  ...
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6", textAlign: "center" }}>
                    {r.AcumulativaCheck ? "Sí" : "No"}
                  </td>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6", textAlign: "center" }}>
                    {r.FijaCheck ? "Sí" : "No"}
                  </td>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6", textAlign: "center" }}>
                    {r.CompuestaCheck ? "Sí" : "No"}
                  </td>

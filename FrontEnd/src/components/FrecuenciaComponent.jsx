// archivo: src/components/FrecuenciaComponent.jsx
...
export default function FrecuenciaComponent() {
  ...
  const loadSituaciones = async () => {
    try {
      const r = await auth.get(`/frecuencia-combos/situacion`);
  ...
  const buildQuery = (withPaging = true) => {
    ...
    if (fId) qs.append("idfrecuencia", fId);
  ...
  const loadRows = async () => {
    try {
      const url = `/frecuencia/?${buildQuery(true)}`;
  ...
  const onDelete = async (id) => {
    ...
    try {
      await auth.delete(`/frecuencia/${id}`);
      await loadRows();
  ...
  const validateClient = () => {
    if (!String(form.Frecuencia || "").trim()) return "Frecuencia es obligatoria.";
    if (String(form.Frecuencia).length > 50) return "Frecuencia: máximo 50 caracteres.";
    if (form.NumeroDias === "" || Number.isNaN(Number(form.NumeroDias))) return "NumeroDias inválido.";
    if (Number(form.NumeroDias) < 0) return "NumeroDias no puede ser negativo.";
    if (form.NumeroHoras === "" || Number.isNaN(Number(form.NumeroHoras))) return "NumeroHoras inválido.";
    if (Number(form.NumeroHoras) < 0) return "NumeroHoras no puede ser negativo.";
    if (!form.PKIDSituacionRegistro) return "Situación es obligatoria.";
    if (form.IDFrecuencia !== "" && !/^-?\d+$/.test(String(form.IDFrecuencia))) return "IDFrecuencia debe ser entero o vacío.";
    return null;
  };
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      IDFrecuencia: form.IDFrecuencia === "" ? null : Number(form.IDFrecuencia),
      Frecuencia: String(form.Frecuencia || "").trim(),
      NumeroDias: Number(form.NumeroDias),
      NumeroHoras: Number(form.NumeroHoras),
      PKIDSituacionRegistro: Number(form.PKIDSituacionRegistro),
    };
    try {
      if (isEditing) {
        await auth.put(`/frecuencia/${form.PKID}`, payload);
        alert("Actualizado.");
      } else {
        await auth.post(`/frecuencia/`, payload);
        alert("Creado.");
      }
      await loadRows();
      onNew();
  ...
  const exportCSV = async () => {
    try {
      ...
      const res = await auth.get(`/frecuencia/export?${qs}`, { responseType: "blob" });
      const blob = new Blob([res.data], { type: "text/csv;charset=utf-8" });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "frecuencia.csv";
      a.click();
  ...
  const exportXLSX = async () => {
    try {
      const qs = buildQuery(false);
      const res = await auth.get(`/frecuencia/export-xlsx?${qs}`, { responseType: "blob" });
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ marginBottom: 12 }}>Frecuencia</h2>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <form onSubmit={onSubmit}>
          <div style={row4}>
            <div>
              <label style={label}>ID Frecuencia</label>
              <input
                name="IDFrecuencia"
                value={form.IDFrecuencia}
                onChange={onChange}
                style={input}
            ...
            <div>
              <label style={label}>Número de Días *</label>
              <input
                type="number"
                name="NumeroDias"
            ...
            <div>
              <label style={label}>Número de Horas *</label>
              <input
                type="number"
                name="NumeroHoras"
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f9fafb" }}>
                ...
                <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #e5e7eb" }}>Frecuencia</th>
                <th style={{ textAlign: "right", padding: 8, borderBottom: "1px solid #e5e7eb" }}># Días</th>
                <th style={{ textAlign: "right", padding: 8, borderBottom: "1px solid #e5e7eb" }}># Horas</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.PKID} style={selected?.PKID === r.PKID ? { background: "#fef3c7" } : undefined}>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.IDFrecuencia}</td>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6" }}>{r.Frecuencia}</td>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6", textAlign: "right" }}>{r.NumeroDias}</td>
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6", textAlign: "right" }}>{r.NumeroHoras}</td>

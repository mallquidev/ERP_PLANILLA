// src/components/DiasUtilesSemanaComponent.jsx
...
export default function DiasUtilesSemanaComponent() {
  ...
  const loadSituaciones = async () => {
    try {
      const res = await auth.get("/dias-utiles-semana-combos/situacion");
  ...
  const loadRows = async () => {
    ...
    try {
      ...
      if (fSemana) params.append("semana", fSemana);
      const res = await auth.get(`/dias-utiles-semana/?${params.toString()}`);
      setRows(res.data || []);
    } catch (err) {
      console.error(err);
  ...
  const onDelete = async (id) => {
    ...
    try {
      setLoading(true);
      await auth.delete(`/dias-utiles-semana/${id}`);
      await loadRows();
      onNew();
    } catch (err) {
  ...
  const onSubmit = async (e) => {
    ...
    if (!form.Ano || !form.Semana) {
      alert("Complete Año y Semana.");
      return;
    }
    // payloads
    const payloadCreate = {
    ...
    try {
      setLoading(true);
      if (isEditing) {
        await auth.put(`/dias-utiles-semana/${form.PKID}`, payloadUpdate);
        alert("Actualizado.");
      } else {
        await auth.post(`/dias-utiles-semana/`, payloadCreate);
        alert("Creado.");
      }
      await loadRows();
      onNew();
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ marginBottom: 16 }}>Días Útiles por Semana</h2>
        ...
        <form onSubmit={onBuscar}>
          <div style={row4}>
            ...
            <div>
              ...
              <input value={fSemana} onChange={(e) => setFSemana(e.target.value)} style={input} />
            </div>
            <div style={{ display: "flex", alignItems: "end", gap: 8 }}>
              ...
              <button
                ...
                onClick={() => { setFAno(""); setFSemana(""); loadRows(); }}
              >
                Limpiar
              </button>
            </div>
        ...
        <form onSubmit={onSubmit}>
          <div style={row4}>
            <div>
              ...
              <input value={empresaNombre ? `${empresaNombre} (ID ${empresaId ?? "-"})` : empresaId ? `Empresa ID ${empresaId}` : "(sin empresa)"} disabled style={{ ...input, background: "#f9fafb" }} />
            </div>
            <div>
              <label style={label}>Año *</label>
            ...
            <div>
              <label style={label}>Semana *</label>
              <input name="Semana" value={form.Semana} onChange={onChange} style={input} />
            </div>
            <div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f9fafb" }}>
                ...
                <th style={{ textAlign: "right", padding: 8, borderBottom: "1px solid #e5e7eb" }}>Semana</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.PKID}>
                  ...
                  <td style={{ padding: 8, borderBottom: "1px solid #f3f4f6", textAlign: "right" }}>{r.Semana}</td>

// archivo: src/components/FechaVigenciaImpuestoComponent.jsx
...
export default function FechaVigenciaImpuestoComponent() {
  const auth = useAuthAxios();

  // filtros
  const [fIdTipo, setFIdTipo] = useState("");
  ...
  const loadSituaciones = async () => {
    try {
      const r = await auth.get(`/fecha-vigencia-impuesto-combos/situacion`);
  ...
  const buildQuery = (withPaging = true) => {
    ...
    if (fIdTipo) qs.append("idtipo", fIdTipo);
    if (fDesde) qs.append("fecha_desde", fDesde);
    if (fHasta) qs.append("fecha_hasta", fHasta);
  ...
  const loadRows = async () => {
    try {
      const url = `/fecha-vigencia-impuesto/?${buildQuery(true)}`;
  ...
  const onClearFilter = () => { setFIdTipo(""); setFDesde(""); setFHasta(""); setFSituacion(""); setPage(1); loadRows(); };
  ...
  const onDelete = async (id) => {
    ...
    try {
      await auth.delete(`/fecha-vigencia-impuesto/${id}`);
      await loadRows();
  ...
  const validateClient = () => {
    const idt = String(form.IDTipoImpuesto || "").trim().toUpperCase();
    if (!idt) return "IDTipoImpuesto es obligatorio.";
    if (idt.length > 3) return "IDTipoImpuesto debe tener máximo 3 caracteres.";
    if (!form.FechaVigencia) return "FechaVigencia es obligatoria.";
    if (form.TasaImpuesto === "" || Number.isNaN(Number(form.TasaImpuesto))) return "TasaImpuesto inválida.";
    if (Number(form.TasaImpuesto) < 0) return "TasaImpuesto no puede ser negativa.";
    if (form.ImporteBase !== "" && Number.isNaN(Number(form.ImporteBase))) return "ImporteBase inválido.";
    if (form.ImporteBase !== "" && Number(form.ImporteBase) < 0) return "ImporteBase no puede ser negativa.";
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      IDTipoImpuesto: String(form.IDTipoImpuesto || "").trim().toUpperCase(),
      FechaVigencia: form.FechaVigencia,
      TasaImpuesto: Number(form.TasaImpuesto),
      ImporteBase: form.ImporteBase === "" ? null : Number(form.ImporteBase),
      PKIDSituacionRegistro: Number(form.PKIDSituacionRegistro),
    };
    try {
      if (isEditing) {
        await auth.put(`/fecha-vigencia-impuesto/${form.PKID}`, payload);
        alert("Actualizado.");
      } else {
        await auth.post(`/fecha-vigencia-impuesto/`, payload);
        alert("Creado.");
      }
      await loadRows();
      onNew();
  ...
  const exportCSV = async () => {
    try {
      ...
      const res = await auth.get(`/fecha-vigencia-impuesto/export?${qs}`, { responseType: "blob" });
      const blob = new Blob([res.data], { type: "text/csv;charset=utf-8" });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "fecha_vigencia_impuesto.csv";
      a.click();
  ...
  const exportXLSX = async () => {
    try {
      const qs = buildQuery(false);
      const res = await auth.get(`/fecha-vigencia-impuesto/export-xlsx?${qs}`, { responseType: "blob" });
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = "fecha_vigencia_impuesto.xlsx";
      a.click();
      window.URL.revokeObjectURL(url);
    } catch {
  ...
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ marginBottom: 12 }}>Fecha Vigencia de Impuestos</h2>
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        <form onSubmit={onSubmit}>
          <div style={row4}>
            <div>
              <label style={label}>ID Tipo Impuesto *</label>
              <input
                name="IDTipoImpuesto"
                value={form.IDTipoImpuesto}
                onChange={onChange}
                style={input}
                maxLength={3}
              />
            ...
            <div>
              <label style={label}>Fecha Vigencia *</label>
              <input
                type="date"
                name="FechaVigencia"
            ...
            <div>
              <label style={label}>Tasa Impuesto *</label>
              <input
                type="number"
                step="0.01"
                name="TasaImpuesto"
            ...
            <div>
              <label style={label}>Importe Base</label>
              <input
                type="number"
                step="0.01"
                name="ImporteBase"
      ...
      <div style={{ ...card, marginBottom: 8 }}>
        <form onSubmit={onFilter}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
            <div>
              <label style={label}>ID Tipo</label>
              <input value={fIdTipo} onChange={(e) => setFIdTipo(e.target.value)} style={input} />
            </div>
            <div>
              <label style={label}>Fecha Desde</label>
              <input type="date" value={fDesde} onChange={(e) => setFDesde(e.target.value)} style={input} />
            </div>
            <div>
              <label style={label}>Fecha Hasta</label>
              <input type="date" value={fHasta} onChange={(e) => setFHasta(e.target.value)} style={input} />
            </div>
            <div>
              <label style={label}>Situación</label>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f9fafb" }}>
                <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #e5e7eb" }}>ID Tipo</th>
                <th style={{ textAlign: "left", padding: 8, borderBottom: "1px solid #e5e7eb" }}>Fecha Vigencia</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.PKID} style={selected?.PKID === r.PKID ? { background: "#fef3c7" } : undefined}>
                  <td style={cell}>{r.IDTipoImpuesto}</td>
                  <td style={cell}>{r.FechaVigencia}</td>
                  <td style={{ ...cell, textAlign: "right" }}>{fmt2(r.TasaImpuesto)}</td>
                  <td style={{ ...cell, textAlign: "right" }}>{fmt2(r.ImporteBase)}</td>
